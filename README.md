...
AI Job Copilot 是一个面向个人求职流程的 AI 面试 Copilot。它把 JD 分析、面试问答、求职记忆沉淀和 RAG 记忆检索串成闭环，让简历项目、回答素材和面试复盘都能变成下一次面试准备的材料。
...
| 使用场景 | JD 分析、面试准备、回答生成、求职记忆沉淀、历史记忆复用 |
| 前端 | 微信小程序、H5 |
...
- **异步 OCR / 链接识别**：支持岗位截图 OCR、岗位链接解析和进度展示，切屏后可继续轮询任务状态。
- **高风险面试雷达**：从 JD 中预判可能被追问的问题，点击后可直接进入 AI 问答。
- **问一问 AI**：针对具体面试问题生成 30 秒短答、完整答法、可引用证据、避坑表达和可能追问。
- **求职记忆库**：将面试复盘、项目经历、回答素材、Mock 反馈、失败问题结构化为可检索的记忆卡片。
- **简历入库召回**：可从简历文本生成多条项目/经历记忆，让简历中的真实证据参与后续 RAG。
- **RAG 召回**：基于当前 JD 或面试问题召回相似求职记忆，辅助生成更贴近个人经历的回答。
...
这个项目里的 RAG 不是传统知识库问答，而是“个人求职记忆检索”。

1. 用户粘贴简历、项目经历、回答素材或面试复盘。
2. LLM 将材料结构化为求职记忆卡片。
3. 后端提取问题、卡点、策略、证据等内容生成 embedding。
4. 记忆卡片写入 Supabase pgvector。
5. 下一次 JD 分析或问一问 AI 时，后端根据当前问题生成 query embedding。
6. Supabase 召回相似历史记忆。
7. DeepSeek 基于当前问题和历史记忆生成回答。

业务价值是：简历里的项目证据、回答素材和每一次复盘都会进入下一次准备，而不是散落在聊天记录和文档里。
...
我做了一个面向个人求职流程的 AI Job Copilot。它不是单纯调用大模型接口，而是把 JD 分析、面试即时问答、求职记忆沉淀和 RAG 记忆检索串成闭环。投递前它会分析岗位匹配度和面试风险；面试前可以基于简历项目和历史记忆生成准备方案；面试中遇到问题可以让 AI 结合 JD 和历史记忆生成回答思路；简历、项目经历和面试复盘都会被结构化成可召回的求职记忆，并通过向量检索在下一次面试中复用。这个项目的重点是让 AI 基于用户的真实经历辅助表达，而不是编造经历。
